...
const rolUsuario = computed(() => authStore.usuario?.rol?.toUpperCase() || '');

const puedeCambiarProyecto = computed(() => ['ADMIN', 'DIRECTIVO', 'SOPORTE'].includes(rolUsuario.value));
const puedeVerReportes = computed(() => ['ADMIN', 'DIRECTIVO'].includes(rolUsuario.value));
const puedeVerConfiguracion = computed(() => ['ADMIN', 'DIRECTIVO', 'SOPORTE'].includes(rolUsuario.value));

const seleccionarProyecto = async (proyecto) => {
  if (!puedeCambiarProyecto.value) return; // Doble candado
  ...
  >
    ...
    <div class="pt-6 px-6 flex-1 flex flex-col">
      ...
      <div class="mb-8 relative z-50">
        ...
        <div v-if="puedeCambiarProyecto"
          @click="dropdownProyectoAbierto = !dropdownProyectoAbierto"
          ...
          <ChevronDown class="w-4 h-4 text-slate-400 transition-transform duration-200" :class="{ 'rotate-180': dropdownProyectoAbierto }" />
        </div>

        <div v-else class="w-full bg-slate-50/50 border border-slate-100 text-slate-600 text-sm font-bold rounded-xl px-4 py-3 flex items-center shadow-inner cursor-not-allowed">
          <div class="flex items-center gap-2 truncate opacity-80">
            <Layers class="w-4 h-4 text-slate-400" />
            <span class="truncate">{{ filtros.proyecto || 'Tu Proyecto' }}</span>
          </div>
        </div>

        <div v-if="dropdownProyectoAbierto && puedeCambiarProyecto"
      ...
      <nav class="space-y-1.5 flex-1 relative z-10">
        ...
        <router-link to="/dashboard" @click="menuMovilAbierto = false"
        ...
        <router-link v-if="puedeVerReportes" to="/reportes" @click="menuMovilAbierto = false"
          class="flex items-center px-4 py-3 rounded-xl text-slate-500 font-semibold hover:bg-slate-50 transition-colors group"
          active-class="bg-admira-50 text-admira-600 font-bold">
      ...
      <div v-if="puedeVerConfiguracion" class="mt-auto pt-6 pb-2">
        <router-link to="/configuracion" @click="menuMovilAbierto = false" 
          class="flex items-center px-4 py-3 rounded-xl text-slate-500 font-semibold hover:bg-slate-50 transition-colors group"
          active-class="bg-admira-50 text-admira-600 font-bold">
    ...
    <div class="p-6 border-t border-gray-100 flex justify-between items-center relative z-10 bg-slate-50/50">
      <div class="flex items-center gap-3 overflow-hidden">
        ...
        <div class="truncate">
          <p class="text-sm font-bold text-slate-800 leading-none truncate">{{ authStore.usuario?.nombre || 'Usuario' }}</p>
          <p class="text-[10px] text-slate-400 mt-1 truncate">{{ authStore.usuario?.sub || 'correo@admira.com' }}</p>
        </div>
      </div>
