...
// Estado para controlar abrir/cerrar el nuevo dropdown
const dropdownProyectoAbierto = ref(false);

// Función actualizada para manejar la selección del menú personalizado
const seleccionarProyecto = async (proyecto) => {
    filtros.proyecto = proyecto;
    filtros.pagina = 1; 
    dropdownProyectoAbierto.value = false; // Cerramos el menú
...
<template>
  <aside class="w-72 bg-white h-screen fixed left-0 top-0 border-r border-gray-200 flex flex-col shadow-sm z-10">
    ...
    <div class="p-6 flex-1 flex flex-col">
      <div class="mb-8 relative z-50">
        ...
        >
          ...
          <div 
            v-for="proy in configInicial.proyectos" 
            :key="proy" 
      ...
      <nav class="space-y-1.5 flex-1 relative z-10">
        <router-link to="/" class="flex items-center px-4 py-3 rounded-xl transition-all group text-slate-500 hover:bg-slate-50" active-class="bg-admira-50 text-admira-600 font-bold" exact-active-class="bg-admira-50 text-admira-600 font-bold">
          <LayoutDashboard class="w-5 h-5 mr-3" />
          <span class="text-sm">Dashboard</span>
        </router-link>
        
        <router-link to="/monitoreo" class="flex items-center px-4 py-3 rounded-xl text-slate-500 font-semibold hover:bg-slate-50 transition-colors group" active-class="bg-admira-50 text-admira-600 font-bold">
          <MonitorSmartphone class="w-5 h-5 mr-3" />
          <span class="text-sm">Monitoreo</span>
        </router-link>

        <router-link to="/reportes" class="flex items-center px-4 py-3 rounded-xl text-slate-500 font-semibold hover:bg-slate-50 transition-colors group" active-class="bg-admira-50 text-admira-600 font-bold">
    ...
    <div class="p-6 border-t border-gray-100 flex justify-between items-center relative z-10">
      <div class="flex items-center gap-3">
        <div class="w-9 h-9 rounded-full bg-slate-100 border border-gray-200 flex items-center justify-center text-xs font-bold text-slate-500">
          AD
        </div>
        <div>
          <p class="text-sm font-bold text-slate-800 leading-none">Admin User</p>
          <p class="text-[10px] text-slate-400 mt-1">admin@admira.com</p>
        </div>
      </div>
      <button class="p-2 text-slate-400 hover:text-admira-600 transition-colors rounded-lg hover:bg-admira-50">
